...
const Pindah = () => {
    ...
    const getPindah = async() =>{
        const response = await axios.get("http://localhost:5000/Pindah");
        console.log(response.data)
        setPindah(response.data);
    };

    const deletePindah = async (id) => {
        try {
            await axios.delete(`http://localhost:5000/Pindah/${id}`);
            getPindah();
        } catch (error){
            console.log(error);
        }
    ...
    const handlePrint = () => {
        ...
        const printContent = `
          <html>
            ...
            <body>
              <h1 class="text-2xl font-bold flex justify-center">Data Kepindahan Warga RT 005/014</h1>
      ...
      const createExcelFile = async () => {
        ...
        const headerRow = worksheet.addRow(["No.", "ID", "Nama", "Alasan"]);
        ...
        pindah.forEach((item, index) => {
          const dataRow = worksheet.addRow([index + 1, item.pindahid, item.nama, item.alasan]);
        ...
        link.download = "data_pindah.xlsx";
        ...
        <div className="container scroll-mx-0">
            <div className="bg-white h-screen">
                <div className="inline-flex gap-x-0">
                    ...
                    <div className="w-auto mt-4 ml-4 text-2xl">Kelola Data Pindah<br/>
                        ...
                        <table className="table-auto border border-slate-300 mt-5">
                           <thead className="text-xl bg-blue-200">
                                <tr>
                                    ...
                                    <th className="pr-40">Alasan</th>
                                    <th className="pl-5 pr-5">Action</th>
                                </tr>
                            </thead> 
                            <tbody>
                            {pindah.map((pindah, index) => (
                            <tr key={pindah.id}>
                                <td className="text-sm text-center">{index + 1}</td>
                                <td className="text-sm">{pindah.pindahid}</td>
                                <td className="text-sm">{pindah.nama}</td>
                                <td className="text-sm">{pindah.alasan}</td>
                                <td>
                                <div className="inline-flex">
                                    <Link to={`/EditPindah/${pindah.id}`}><div className="ml-5"><FiEdit /></div></Link>
                                    <button onClick={() => deletePindah(pindah.id)}><div className="ml-3"><RiDeleteBin6Line /></div></button>
                                </div>
                                </td>
                            </tr>
                            ))}
