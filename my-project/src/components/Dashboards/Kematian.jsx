...
const Kematian = () => {
    ...
    const getKematian = async () => {
        const response = await axios.get("http://localhost:5000/Kematian");
        setkematian(response.data);
    };

    const deleteKematian = async (id) => {
        try {
            await axios.delete(`http://localhost:5000/Kematian/${id}`);
            getKematian();
        } catch (error){
            console.log(error);
        }
    ...
    const handlePrint = () => {
        ...
        const printContent = `
          <html>
            ...
            <body>
              <h1 class="text-2xl font-bold flex justify-center">Data Kematian Warga RT 005/014</h1>
      ...
      const createExcelFile = async () => {
        ...
        const headerRow = worksheet.addRow(["No.", "NIK", "Nama", "Penyebab"]);
        ...
        kematian.forEach((item, index) => {
          const dataRow = worksheet.addRow([index + 1, item.nik, item.nama, item.penyebab]);
        ...
        link.download = "data_kematian.xlsx";
        ...
        <div className="container scroll-mx-0">
            <div className="bg-white h-screen">
                <div className="inline-flex gap-x-0">
                    ...
                    <div className="w-auto mt-4 ml-4 text-2xl">Kelola Data Kematian<br />
                        ...
                        <table className="table-auto border border-slate-300 mt-5">
                           <thead className="text-xl bg-blue-200">
                                <tr>
                                    ...
                                    <th className="pr-40">Penyebab</th>
                                    <th className="pl-5 pr-5">Action</th>
                                </tr>
                            </thead> 
                            <tbody>
                                {kematian.map((kematian, index) => (
                                <tr key={kematian.id}>
                                    <td className="text-sm flex justify-center pt-2">{index + 1}</td>
                                    <td className="text-sm">{kematian.nik}</td>
                                    <td className="text-sm">{kematian.nama}</td>
                                    <td className="text-sm">{kematian.penyebab}</td>
                                    <td>
                                        <div className="inline-flex">
                                            <Link to={`/EditKematian/${kematian.id}`}><div className="ml-5"><FiEdit /></div></Link>
                                            <button onClick={() => deleteKematian(kematian.id)}><div className="ml-3"><RiDeleteBin6Line /></div></button>
                                        </div>
                                    </td>
                                </tr>
                                ))}
